...
blur_gray = cv2.blur(gray,(3,3)) # blur to be able to detect all the edges in the image clearly 
edges = cv2.Canny(puzzle,30,90,apertureSize = 3) 

lines = cv2.HoughLines(edges,2,np.pi/180,300,0,0) #200 pixels is set to be the minimum length of the line to be detected 
lines = sorted(lines, key=lambda line:line[0][0]) 		# sort the lines based on the value of ppd
h_list, v_list = flag_lines(lines,puzzle) # draw detected lines to see whether lines have been detected .. 

draw_detected(lines,puzzle)
points = get_intersection(h_list,v_list)
print("Total points detected -> ",len(points))

if len(points) == 100 : # since number of points have to be 100 
	bin_img = cv2.adaptiveThreshold(blur_gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,101,1) # convert to binary image 
